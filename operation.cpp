...
void Operation::Do() {
    ...
    std::cout << "Mission impossible. Out of work area." << std::endl;
}
...
void Operation::ShowOperation() const {
    int missionIndex = 0;
    for (const auto &mission : missionsList) {
        std::cout << "Mission " << missionIndex++ << std::endl;
...
int Operation::getOperationLen() const {
    return missionsList.size();
}

int Operation::getDeletedOperationLen() const {
    return deletedMissionsList.size();
}

void Operation::SwapMissions(int idx1, int idx2) {
    int operationLen = missionsList.size();
    if (idx1 >= 0 && idx2 >= 0 && idx1 < operationLen && idx2 < operationLen) {
        std::swap(missionsList[idx1], missionsList[idx2]);
        UpdateFileWithOperationData();
        return;
    }
    std::cout << "Incorrect missions' index" << std::endl;
}

void Operation::DeleteMission(int idx) {
...
void Operation::InsertMission(int idx, std::unique_ptr<IMission> mission) {
...
void Operation::PushDeletedMission(int idx) {
    if (idx >= 0 && idx < missionsList.size()) {
